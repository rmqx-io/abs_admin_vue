<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="handleAdd">新增智能灯型号</a-button>
...
import { getSmartLightModelList, addSmartLightModel, updateSmartLightModel, deleteSmartLightModel } from '@/api/smartLight'
import CreateSmartLightForm from './CreateSmartLightForm'
import SmartLightDetailModal from './SmartLightDetailModal'
...
export default {
  ...
  components: {
    STable,
    CreateSmartLightForm,
    SmartLightDetailModal
  },
  data () {
    ...
    return {
      ...
      queryData: {
        name: null,
        manufacturer: null,
        device_type: 'smart_light', // 固定为智能灯
        page_no: 1,
        page_size: 10
      },
      loadData: parameter => {
        ...
        return getSmartLightModelList(requestParameters)
          .then(res => {
            ...
            console.error('Failed to load smart light models:', err)
  ...
  methods: {
    ...
    handleDelete (id) {
      deleteSmartLightModel(id).then(res => {
    ...
    handleBatchDelete () {
      ...
      this.$confirm({
        ...
        onOk: () => {
          const deletePromises = this.selectedRowKeys.map(id => deleteSmartLightModel(id))
    ...
    handleOk () {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          ...
          const request = values.id ? updateSmartLightModel(values) : addSmartLightModel(values)
