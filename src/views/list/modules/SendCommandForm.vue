<template>
  <a-modal title="下发指令" width="90vw" centered :visible="visible" :confirmLoading="loading" @ok="() => { handleOk() }"
    @cancel="() => { handleCancel(); $emit('cancel') }">
    <select-device-ids :deviceIds="deviceIds" :showAdd=false />
    <a-table :dataSource="sendCommandList" :columns="columns" rowKey="name" :customRow="customRow" />
    <el-row :gutter="24">
      <el-col :span="5">
        <a-input style="width: 150px;" v-model="countdown" disabled></a-input>
        倒计时(秒)
      </el-col>
      <el-col :span="6">
        <a-button type="primary" @click="startTimer">定时升级</a-button>
      </el-col>
    </el-row>
    <a-form-item v-if="currentRow" :label="currentRow.name_cn">
      <!-- create a a-input for each currentRow.params -->
      <!-- loop with index -->
      <p v-if="showHexEdit">字符串辅助填写</p>
      <a-input v-if="showHexEdit" v-model='help_param' placeholder="16进制字符串自动拆分填充" @change="helpInputParams"></a-input>
      <a-form-item v-if="currentRow.params" v-for="(p, i) in currentRow.params" :key="p.name" :label="p.name">
        ...
        <a-row :gutter="48">
          <a-col :md="12" :sm="24">
            <a-input v-model='decimal_params[i]' placeholder="输入参数" @change="handleDecimalParamChange(i)"></a-input>
          </a-col>
          <a-col :md="4" :sm="12">
            <span>{{ p.unit }}</span>
          </a-col>
        ...
        <a-input v-if="showHexEdit" v-model='params[i]' placeholder="00 00" @change="handleParamChange(i)"></a-input>
      </a-form-item>
      <p v-if="currentRow.params">根据上面的参数拼接的结果：</p>
      <a-input v-model='param' placeholder="输入参数"></a-input>
    </a-form-item>
    <a-form-item>
      <a-checkbox v-model:checked="enableOfflineMessage" @change="handleSaveOfflineMessageChange">离线设备自动重发</a-checkbox>
      <a-checkbox v-model="showHexEdit">16进制编辑</a-checkbox>
    </a-form-item>
    <a-form-item v-if="enableOfflineMessage">
      <a-date-picker v-model="offlineMessageTtl" :showTime="true" :format="'YYYY-MM-DD HH:mm:ss'"
  ...
  export default {
    ...
    methods: {
      refreshTable(param) {
        ...
        getDeviceList(arg)
          .then(res => {
            // console.log('device list', res.data.records)
            that.DeviceList = res.data.records
          })
      },
      startTimer() {
        // 清除之前的定时器（如果有的话）  
        // console.log("设备编号：", this.deviceIds.length);
        const chunkSize = 10;
        for (let i = 0; i < this.deviceIds.length; i += chunkSize) {
          this.chunkedDeviceIds.push(this.deviceIds.slice(i, i + chunkSize));
        }
        // console.log("设备编号：", chunkedDeviceIds);
        this.stopTimer();
        // 获取当前日期和时间  
        const now = new Date();

        // 设置目标日期为今晚凌晨两点  
        const targetDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + (now.getHours() >= 2 ? 1 :
            0), 2,
          0, 0);

        // 如果目标时间已经过去，则设置为明天凌晨两点  
        if (targetDate < now) {
        ...
        this.timerId = setTimeout(() => {
          // 这里写你要在凌晨两点执行的代码  
          console.log('两点到了！');
          that.processChunkedDeviceIds(this.chunkedDeviceIds, 0, 5000);
        }, delay);
      },
      processChunkedDeviceIds(chunkedDeviceIds, index, delay) {
        let that = this;
        if (index >= chunkedDeviceIds.length) {
          setTimeout(() => {
            // 所有分块都处理完了
            that.refreshTable()
            console.log("deviceList3", that.DeviceList.length)
            for (let i = 0; i < that.DeviceList.length; i++) {
              if (that.DeviceList.bms_software_version != "103") {
                that.deviceIds.push({
                  deviceId: that.DeviceList.code
                })
                let j = 0
                console.log("失败：", j + 1);
              }
        ...
        // 处理当前分块的设备ID  
        this.deviceIds = chunkedDeviceIds[index];
        this.handleOk();

        // 递归调用自己，处理下一个分块，并加上延时  
        setTimeout(() => {
          that.processChunkedDeviceIds.call(this, chunkedDeviceIds, index + 1, delay);
        }, delay);
      },
      stopTimer() {
        if (this.timerId) {
