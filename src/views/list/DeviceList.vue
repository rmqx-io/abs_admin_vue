<template>
  ...
  <a-card :bordered="false" :bodyStyle="{ padding: '16px 16px', height: '100%' }" :style="{ height: '100%' }">
    <a-spin v-if='showExporting' tip="导出中...">
      <a-alert message="导出设备" description="导出选中的设备"></a-alert>
    </a-spin>
    <div class="table-page-search-wrapper">
      ...
      <a-form>
        <a-row :gutter="48">
          ...
          <a-col :md="4" :sm="12"><a-form-item :label="$t('No')"><a-input v-model.trim="queryData.device_id"
                placeholder="" /></a-form-item></a-col>
          <a-col :md="4" :sm="12">
            <a-form-item :label="$t('Organization')">
              <a-tree-select show-search tree-default-expand-all :filterTreeNode="filterTreeNode" :treeData="orgList"
                v-model="queryData.organization_id"></a-tree-select>
            </a-form-item>
          </a-col>
          <a-col :md='4' :sm='12'>
            ...
            <div v-if='showMoreParam'>
              <a class="ant-dropdown-link"
                @click='() => { showMoreParam = false; queryData.bt_code = ""; queryData.iccid = ""; queryData.soh = ""; queryData.soc = ""; queryData.alarm = "" }'>收起参数<a-icon
                  type="up" /></a>
            </div>
          </a-col>
        </a-row>
        <a-row :gutter='48'>
          <div v-if='showMoreParam'>
            <a-col :md='4' :sm='12'><a-form-item :label="$t('BT Code')"><a-input v-model="queryData.bt_code"
                  placeholder="" /></a-form-item></a-col>
            <a-col :md='4' :sm='12'><a-form-item :label="$t('ICCID')"><a-input v-model="queryData.iccid"
                  placeholder="" /></a-form-item></a-col>
            <a-col :md='4' :sm='12'><a-form-item :label="$t('SOH')"><a-input v-model="queryData.soh"
                  placeholder="" /></a-form-item></a-col>
            <a-col :md='4' :sm='12'><a-form-item :label="$t('SOC')"><a-input v-model="queryData.soc"
                  placeholder="" /></a-form-item></a-col>
    ...
    <div v-if='showTableTab'>
      <div v-if="table_visible" class="table-operator">
        ...
        <a-button type='primary' @click='handleSendCommandBatch'>下发指令</a-button>
      ...
      <s-table v-if="table_visible" ref="table" size="default" :rowKey="(record) => record.id" :columns="columns"
        :data="loadData" :alert="true" :rowSelection="rowSelection" showPagination="auto" :scroll="{ x: 1300 }">
        ...
        <span slot="device_status" slot-scope="text, record">
          <template>
            状态：<span>{{record.deviceStatus}}</span>
          </template>
        </span>
      ...
      <create-form v-if="table_visible" ref="createModal" :visible="device_create_form_visible"
        :loading="confirmLoading" :model="device_create_form_data" @cancel="handleCreateFormCancel"
        @ok="handleCreateFormOk" />

      <send-command-form ref="sendCommandModal" :visible="send_command_form_visible" :loading="confirmLoading"
        :model="send_command_form_data" :device-ids="device_ids" @cancel="handleSendCommandFormCancel"
        @ok="handleSendCommandFormOk" />

      <send-command-manager ref='sendCommandManager' :visible="showBatchCommandManager" :loading="false"
        @cancel="handleSendCommandManagerCancel" @ok="handleSendCommandManagerOk" />

      <send-bt-code ref='sendBtCode' :visible="send_bt_code_visible" :loading="false" @cancel="handleSendBtCodeCancel"
        @ok="handleSendBtCodeOk" />

      <send-command-manager-devices ref="sendCommandManagerDevices" :visible="showBatchCommandManagerDevices"
        :currentBatchSendCommandId='currentBatchSendCommandId' @ok="() => { showBatchCommandManagerDevices = false }"
      ...
      <battery-info v-if="battery_detail_visible" ref="batteryInfo" :device-id="device_id" :bms-bt="bms_bt"
        @cancel="handleBatteryInfoCancel" @ok="handleBatteryInfoOk" />

      <div v-if="map_visible" style="width: 100%">
        <div><a @click="handleMapClose()">
            << 返回</a>
        </div>
        <div><br /></div>
        ...
        <amap :zoom="zoom" :center="center" style='height: 70vh'>
          <amap-polyline :path="polyline.path" :line-join="'round'">
          </amap-polyline>
          <amap-circle-marker v-for="(marker,i) in polyline.markers" :center="marker" :key="i" :radius="5">
          </amap-circle-marker>
        </amap>
      </div>
      <div>
    ...
    <div v-if='packet_log_visible'>
      <a-modal title="协议日志" width='90vw' centered :visible="packet_log_visible" @cancel='packet_log_visible = false'
        @ok='packet_log_visible = false'>
        <packet-log ref='protocolLog' :visible="packet_log_visible" :device-id='device_id'
          @cancel='handlePacketLogCancel' @ok='handlePacketLogOk' />
      </a-modal>
    </div>

    <div v-if="showMap" style="width: 100%; height: 70vh" class="map-container">
      <div v-if="isGettingDeviceLocation" class='popup'>
        <a-progress :percent="getDevicesLocationPageNo/ getDevicesLocationPages * 100"
          :status="getDevicesLocationPageNo === getDevicesLocationPages ? 'success' : 'active'" :stroke-width="10"
          :format="percent => `${percent.toFixed(0)}%`" />
        <span>{{ getDevicesLocationPageNo }}</span> / <span>{{ getDevicesLocationPages }}</span>,
        <span>{{ markersFound }}</span>
      </div>
      <!-- TODO: use device map component -->
      <amap :center="center2" :zoom="4" cache-key="marker-cluster-map" async v-if="refresh_map">
        <!--        <template>-->
        <!--          <amap-marker-->
        ...
        <!--        </template>-->
        <!-- 点聚合 -->
        <amap-marker-cluster v-if="showMarkers" :data="deviceMarkers" key="custom-cluster" :grid-size="options.gridSize"
          :average-center="options.averageCenter" :marker-options="getMarkerOptions">
    ...
    <div v-if="showAlarm" class="alarm-container">
      <device-alarm ref='alarm' :device-id="queryData.device_id" :device-status="deviceStatus"
...
<script>
  ...
  const columns = [
    ...
    },
    {
      ...
      scopedSlots: {
        customRender: 'organization_info'
      }
    },
    {
      title: 'BMS',
    ...
    data() {
      ...
      return {
        ...
        data: [{
          ...
          {
            lnglat: [113.929379, 22.532922],
            weight: 40
          },
          {
            lnglat: [113.928178, 22.531258],
            weight: 40
          }
        ],
        options: {
        ...
        markers2: [{
          ...
          {
            lnglat: [116.397888, 39.900168],
            label: 'Marker 2'
          },
          {
            lnglat: [116.410332, 39.89734],
            label: 'Marker 3'
          }
        ],
        ...
        markers: [{
            position: [116.402144, 39.910012],
            title: 'Marker 1'
          },
          {
            position: [116.391095, 39.904684],
            title: 'Marker 2'
          },
          {
            position: [116.418044, 39.957106],
            title: 'Marker 3'
          },
          {
            position: [116.373688, 39.931149],
            title: 'Marker 4'
          }
        ],
        deviceMarkers: [],
