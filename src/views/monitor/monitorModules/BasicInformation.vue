  ...
  <a-card>
    <el-row>
      <el-col type="flex" align="middle" :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
        <div class="content_bt" style="width: 380px; margin-top: 16px">
          <span style="
              font-size: 14px;
              font-weight: bold;
              text-align: left;
            ...
            <el-tag :type="this.battery_currency === 0 ? 'info' : this.battery_currency > 0 ? 'warning' : 'danger'">
        ...
        <div>
          ...
          <el-image style="margin-top: 15px; width: 300px; height: 160px" class="img_battery" fit="fill"
        ...
        <el-row type="flex" align="middle" style="margin-top: 20px">
          ...
          <el-col :span="10" type="flex" align="middle">
            <el-progress :text-inside="true" :stroke-width="20" :percentage="battery_healthy"
              :color="customColorMethod"></el-progress>
          </el-col>
        </el-row>

        <el-col :span="24" type="flex" align="middle" justify="middle" style="margin-top: 30px; margin-bottom: 30px">
          <el-descriptions :column="3" border>
            <el-descriptions-item label="充电MOS" span="1">
              <el-switch v-model="isMosRec" disabled active-color="#41b584" inactive-color="#f34d37"></el-switch>
            </el-descriptions-item>
            <el-descriptions-item label="放电MOS" span="1">
              <el-switch v-model="isMosDis" disabled active-color="#41b584" inactive-color="#f34d37"></el-switch>
            </el-descriptions-item>
            <el-descriptions-item label="均衡开关" span="1">
              <el-switch v-model="isBalanceOn" disabled active-color="#41b584" inactive-color="#f34d37"></el-switch>
            </el-descriptions-item>
        ...
        <el-row type="flex" align="middle" style="padding: 8px">
          <el-col :span="12" type="flex" align="middle">
            <span>放电MOS:</span>
            <el-tag size="mini" type="danger" effect="dark" @click="controlMos('discharge', '00')">
              关闭
            </el-tag>
            <el-tag size="mini" type="success" effect="dark" @click="controlMos('discharge', '01')">
              开启
            </el-tag>
          </el-col>
          <el-col :span="12" type="flex" align="middle">
            <span>充电MOS:</span>
            <el-tag size="mini" type="danger" effect="dark" @click="controlMos('charge', '00')">
              关闭
            </el-tag>
            <el-tag size="mini" type="success" effect="dark" @click="controlMos('charge', '01')">
              开启
            </el-tag>
          </el-col>
        </el-row>
      ...
      <el-col type="flex" align="middle" :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
        ...
        <el-row>
          <el-col type="flex" align="middle" justify="middle">
            <chart-bms-more ref="bmschart"></chart-bms-more>
            <span v-if="initHis && dataListBMS.length === 0"
              style="border-style: solid; border-width: 1px; padding: 8px">
              设备无数据
            </span>
          </el-col>
          <el-col :span="23" :offset="1" type="flex" align="middle" justify="middle">
            <el-row v-for="(itemList, indexList) in tempList" :key="'temp' + indexList" style="margin-top: 8px"
              type="flex" align="middle">
              <el-col v-for="(item, index) in itemList" :key="'temp2' + index" :span="7" class="content_center">
                <el-image class="item_battery" :src="require('@/assets/battery/icon_temp_item.png')" />
            ...
            <!-- 单个电池 -->
            <el-row v-for="(itemList, indexList) in batteryList" :key="'battery' + indexList" style="margin-top: 8px"
              type="flex">
              <el-col v-for="(item, index) in itemList" :key="'battery2' + index" :span="5" class="content_start">
                <span style="font-size: 12px; width: 20px">
                  [{{ indexList * 5 + index + 1 }}]
                </span>
                <el-image class="item_battery" :src="require('@/assets/battery/icon_battery_item.png')" />
                <span :style="
    ...
    <el-row>
      <!-- 基础信息 -->
      <el-row type="flex" justify="space-between" align="middle" style="padding: 8px; margin-top: 8px">
        <el-col :span="20">
      ...
      <el-descriptions :column="6" size="mini" border>
        ...
        <el-descriptions-item span="2">
          ...
          <el-tag :type="battery_currency === 0 ? 'info' : battery_currency > 0 ? 'warning' : 'danger'" size="mini">
    ...
    <el-row>
      ...
      <el-descriptions :column="10" size="mini" border>
        <el-descriptions-item span="2" v-for="item in alarms" :key="item.key" :label="item.key">
    ...
    <el-row :gutter="48">
      <el-col :md="8" :sm="24">
        <div class="block">
          <el-date-picker v-model="queryData.start_data" type="date" placeholder="起始日期">
          </el-date-picker>
        </div>
      </el-col>
      <el-col :md="8" :sm="24">
        <el-time-select v-model="queryData.start_time" :picker-options="{
            start: '08:30',
            step: '00:15',
            end: '18:30'
          }" placeholder="起始时间">
    ...
    <el-row :gutter="48" style="margin-top: 30px;">
      <el-col :md="8" :sm="24">
        <div class="block">
          <el-date-picker v-model="queryData.end_data" type="date" placeholder="结束日期">
          </el-date-picker>
        </div>
      </el-col>
      <el-col :md="8" :sm="24">
        <el-time-select v-model="queryData.end_time" :picker-options="{
            start: '08:30',
            step: '00:15',
            end: '18:30'
          }" placeholder="结束时间">
        </el-time-select>
        <a-button type="primary" style="margin-left: 40px;">查询</a-button>
      </el-col>
    </el-row>
    <template>
      <a-table :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }" :columns="columns"
        :data-source="data" style="margin-top: 30px;" />
    </template>
  </a-card>
...
<script>
  ...
  const columns = [{
    ...
    },
    {
      ...
      scopedSlots: {
        customRender: 'single_battery_voltage'
      }
    },
    {
      title: 'SOC',
      ...
      scopedSlots: {
        customRender: 'battery_capacity_soc'
      }
    },
    {
      title: '电池电压',
  ...
  for (let i = 0; i < 10086; i++) {
    data.push({
      id: 1 + i,
      single_battery_voltage_arr: `Edward King ${i}`,
      battery_capacity_soc: "100%",
      battery_voltage: `London, Park Lane no. ${i}`,
    });
  }
  ...
  export default {
    ...
    methods: {
      ...
      set_alarms() {
        this.alarms = [{
            key: '低容量报警',
            value: this.battery_alarm_low_power
          },
          {
            key: 'MOS管超温报警',
            value: this.battery_alarm_mos_high_temperature
          },
          {
            key: '充电过压报警',
            value: this.battery_alarm_charging_high_voltage
          },
          {
            key: '放电欠压报警',
            value: this.battery_alarm_discharging_low_voltage
          },
          {
            key: '电池超温报警',
            value: this.battery_alarm_high_temperature
          },
          {
            key: '充电过流报警',
            value: this.battery_alarm_charging_high_current
          },
          {
            key: '放电过流报警',
            value: this.battery_alarm_discharging_high_current
          },
          {
            key: '电芯压差报警',
            value: this.battery_alarm_voltage_difference
          },
          {
            key: '电池箱内超温报警',
            value: this.battery_alarm_box_high_temperature
          },
          {
            key: '电池低温报警',
            value: this.battery_alarm_low_temperature
          },
          {
            key: '单体过压报警',
            value: this.battery_alarm_single_cell_high_voltage
          },
          {
            key: '单体欠压报警',
            value: this.battery_alarm_single_cell_low_voltage
          },
          {
            key: '309_A保护',
            value: this.battery_alarm_309_a
          },
          {
            key: '309_B保护',
            value: this.battery_alarm_309_b
          },
          {
            key: '湿度报警',
            value: this.battery_alarm_humidity
          },
          {
            key: '防拆报警',
            value: this.battery_alarm_disassemble
          }
        ]
      },
          ...
          getBatteryInfoLatest(this.deviceId, res.data.bms_type, {})
            .then(res => {
              ...
              if (res.data && res.data.vehicle_detail_vo) {
                ...
                this.gps_location_time = moment(res.data.vehicle_detail_vo.time_tracking).format(
                  'YYYY-MM-DD HH:mm:ss')
              ...
              if (res.data && res.data.logs && res.data.logs.length > 0) {
                ...
                this.batteryList = this.form_battery_voltage_array_for_display(bmsInfo.single_battery_voltage_arr
                  .split(','), 5)
