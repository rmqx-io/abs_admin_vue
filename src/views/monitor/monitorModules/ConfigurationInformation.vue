<template>
  <el-card>
    <div>
      ...
      <!--<div>bms config</div>-->
      <a-modal :visible="showSetConfigForm" @ok="() => { handleOk() }" @cancel="() => { handleCancel() }">
        <h3>修改 BMS 配置</h3>
        <a-form>
      ...
      <el-descriptions :column="6" size="mini" border>
        <el-descriptions-item span="2" v-for="(value, name, index) in this.bmsConfig" :key="name"
          :label="bmsConfigKeyToNameFromServer[name].name_cn">
          <span>{{ value }}</span>
          <br />
          <template v-if='bmsConfigKeyToNameFromServer[name].writable'><a
              @click='handleSetBmsConfig(bmsConfigKeyToNameFromServer[name], value)'>配置</a></template>
...
<script>
  import {
    ...
    getBmsConfigDataTypes,
    getBmsConfigDataTypesMap,
    getBmsType
  } from '@/api/manage'
