<template>
  <a-modal title="添加组织" :width="640" :visible="visible" :confirmLoading="loading" @ok="() => { $emit('ok') }"
    @cancel="() => { $emit('cancel') }">
      ...
      <a-form :form="form" v-bind="formLayout">
        ...
        <a-form-item label="上级组织">
          <a-tree-select show-search tree-default-expand-all :filterTreeNode="filterTreeNode" :treeData="orgList"
            v-decorator="['parent_id', { rules: [ {required: true} ]}]"></a-tree-select>
        </a-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
      console.log('create org modal created')
